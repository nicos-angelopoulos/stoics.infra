/** kvs_k_update_v( +KVset, +K, +GoalNew, GoalUpd, -V, -NewV, -NVset ).

Update or insert the V part of two arg sorted terms list KVset to NVset.

GoalNew is called if K does not appear in KVset and GoalUpd when K does exist.
First is via =|call(+GoalNew, +K, -NewV, -NewKV)|= and second via =|call( +GoalUpd, +K, +V, -NewV, -NewKV )|=.
...
?- kvs_k_update_v( [a-1,b-2,c-2], d, v_val(0), v_plus(1), V, Nv, Set ).
Nv = 0,
Set = [a-1, b-2, c-2, d-0].

?- kvs_k_update_v( [a-1,b-2,c-2], 0, v_val(1), v_plus(1), V, Nv, Set ).
Nv = 1,
Set = [0-1, a-1, b-2, c-2].
==
